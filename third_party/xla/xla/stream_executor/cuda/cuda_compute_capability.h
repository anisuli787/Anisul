...
struct CudaComputeCapability {
  ...
  // Has the same semantics as `SupportsAllFeaturesOf` and mainly a remnant from
  // the times when compute capabilities could be totally ordered. It is
  // recommended to only use this function when checking for features that are
  // not from the accelerated or forward compatible feature sets. Use `CanRunOn`
  // or `SupportsAllFeaturesOf` to make the code more readable when checking for
  // sm_X.Ya or sm_X.Yf features.
  bool IsAtLeast(const CudaComputeCapability& other) const {
    return SupportsAllFeaturesOf(other);
  }

  // Convenience overload for checking for a specific major and minor version.
  // See `IsAtLeast(const CudaComputeCapability&)` for more details on the
  ...
  bool IsAtLeastAda() const {
    constexpr int kAdaMinor = 9;
    return SupportsAllFeaturesOf(CudaComputeCapability{
        CudaComputeCapabilities::kAmpere, kAdaMinor, FeatureExtension::kNone});
  }
  ...
  bool IsPascalGeneration() const {
    return major == CudaComputeCapabilities::kPascal;
  }

  bool IsVoltaGeneration() const {
    return major == CudaComputeCapabilities::kVolta;
  }

  bool IsAmpereGeneration() const {
    return major == CudaComputeCapabilities::kAmpere;
  }

  bool IsHopperGeneration() const {
    return major == CudaComputeCapabilities::kHopper;
  }

  bool IsBlackwellGeneration() const {
    return major == CudaComputeCapabilities::kBlackwell;
  }
