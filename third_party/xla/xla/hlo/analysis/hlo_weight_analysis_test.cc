...
class HloWeightAnalysisTest : public HloHardwareIndependentTestBase {
 public:
  bool IsWeight(const HloWeightAnalysis& hlo_weight_analysis, HloModule* module,
    ...
    std::optional<ShapeTree<WeightInfo>> weight_info =
        hlo_weight_analysis.GetWeightInfo(instruction);
    return weight_info.has_value() &&
           (*weight_info).element({}) == WeightInfo::kWeight;
  }
};

TEST_F(HloWeightAnalysisTest, OneMatmul) {
...
ENTRY entry {
  ...
  TF_ASSERT_OK_AND_ASSIGN(
      std::unique_ptr<HloWeightAnalysis> hlo_weight_analysis,
      HloWeightAnalysis::Run(*module));
  EXPECT_TRUE(IsWeight(*hlo_weight_analysis, module.get(), "copy"));
  EXPECT_TRUE(IsWeight(*hlo_weight_analysis, module.get(), "Arg_1.2"));
}

TEST_F(HloWeightAnalysisTest, RepeatWhile) {
  const std::string module_str = R"(
...
ENTRY entry {
  ...
  TF_ASSERT_OK_AND_ASSIGN(
      ...
      HloWeightAnalysis::Run(*module));
  EXPECT_TRUE(
      IsWeight(*hlo_weight_analysis, module.get(), "get-tuple-element.55"));
  EXPECT_TRUE(IsWeight(*hlo_weight_analysis, module.get(), "reshape.74"));
  EXPECT_TRUE(IsWeight(*hlo_weight_analysis, module.get(), "reshape.79"));
  EXPECT_TRUE(IsWeight(*hlo_weight_analysis, module.get(), "reshape.22"));
  EXPECT_TRUE(IsWeight(*hlo_weight_analysis, module.get(), "reshape.23"));
  EXPECT_TRUE(IsWeight(*hlo_weight_analysis, module.get(), "reshape.24"));
}
