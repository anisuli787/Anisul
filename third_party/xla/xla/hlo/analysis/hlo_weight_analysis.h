...
#ifndef XLA_HLO_ANALYSIS_HLO_WEIGHT_ANALYSIS_H_
#define XLA_HLO_ANALYSIS_HLO_WEIGHT_ANALYSIS_H_
...
// This analysis pass that determine which HLO instructions produce/are weights.
...
// etc).
class HloWeightAnalysis {
 public:
  friend class HloWeightPropagation;
  static absl::StatusOr<std::unique_ptr<HloWeightAnalysis>> Run(
...
class HloWeightPropagation : public DfsHloVisitorWithDefault {
 public:
  explicit HloWeightPropagation(HloWeightAnalysis* weight_analysis)
      : analysis_(weight_analysis) {}
  ...
  absl::Status HandleSimpleNoop(HloInstruction* noop);
 ...
 protected:
  HloWeightAnalysis* analysis_;
};

}  // namespace xla

#endif  // XLA_HLO_ANALYSIS_HLO_WEIGHT_ANALYSIS_H_
