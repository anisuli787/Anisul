...
#include "xla/hlo/analysis/hlo_weight_analysis.h"
...
bool HloWeightAnalysis::IsInstructionWeight(
...
std::optional<ShapeTree<WeightInfo>> HloWeightAnalysis::GetWeightInfo(
    const HloInstruction* instruction) const {
  auto it = info_map_.find(instruction);
  if (it == info_map_.end()) {
    return std::nullopt;
...
absl::Status HloWeightAnalysis::SetInstructionAsWeight(
...
absl::Status HloWeightAnalysis::SetWeightInfo(
...
absl::Status HloWeightAnalysis::AnnotateEntryComputationParameters(
...
absl::StatusOr<std::unique_ptr<HloWeightAnalysis>> HloWeightAnalysis::Run(
    const HloModule& module,
    const absl::flat_hash_set<absl::string_view>& execution_threads) {
  std::unique_ptr<HloWeightAnalysis> weight_analysis =
      absl::WrapUnique(new HloWeightAnalysis(module, execution_threads));
  RETURN_IF_ERROR(weight_analysis->AnnotateEntryComputationParameters(module));
  RETURN_IF_ERROR(
      weight_analysis->RunOnComputation(*module.entry_computation()));
  return weight_analysis;
}

absl::Status HloWeightAnalysis::RunOnComputation(
...
absl::Status HloWeightAnalysis::RunOnComputation(
  ...
  for (int i = 0; i < computation.num_parameters(); ++i) {
    ...
    RETURN_IF_ERROR(SetWeightInfo(computation.parameter_instructions()[i],
...
absl::Status HloWeightPropagation::HandleTuple(HloInstruction* tuple) {
  ...
  if (has_weight_info) {
    RETURN_IF_ERROR(analysis_->SetWeightInfo(tuple, std::move(weight_tree)));
  }
...
absl::Status HloWeightPropagation::HandleCall(HloInstruction* call) {
  ...
  if (analysis_->IsInstructionWeight(computation->root_instruction())) {
    RETURN_IF_ERROR(analysis_->SetWeightInfo(
...
absl::Status HloWeightPropagation::HandleSimpleNoop(HloInstruction* noop) {
  RETURN_IF_ALREADY_PROPAGATED(noop);
  const HloInstruction* operand = noop->operand(0);
  if (analysis_->IsInstructionWeight(operand)) {
    RETURN_IF_ERROR(analysis_->SetInstructionAsWeight(noop));
  }
  return absl::OkStatus();
}
...
absl::Status HloWeightPropagation::HandleDynamicSlice(
  ...
  return HandleSimpleNoop(dynamic_slice);
}
