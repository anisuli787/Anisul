...
void convertManualComputationOp(
  ...
  for (auto [globalOperand, localArgumentType, inSharding] :
    ...
    if (createHloShardingConstraints) {
      auto customCallOp = rewriter.create<mlir::stablehlo::CustomCallOp>(
          loc, globalOperand.getType(), globalOperand);
      customCallOp.setCallTargetName(kShardingCustomCallTargetName);
      shardingConstraint = customCallOp;
    } else {
      shardingConstraint = rewriter.create<CopyOp>(loc, globalOperand);
    }
    ...
    auto fullToShard = rewriter.create<CustomCallOp>(
        loc, localArgumentType, shardingConstraint->getResult(0));
  ...
  for (auto [localResult, oldResult, outSharding] :
    ...
    if (createHloShardingConstraints) {
      auto customCallOp = rewriter.create<mlir::stablehlo::CustomCallOp>(
          loc, localResult.getType(), localResult);
      customCallOp.setCallTargetName(kShardingCustomCallTargetName);
      shardingConstraint = customCallOp;
    } else {
      shardingConstraint = rewriter.create<CopyOp>(loc, localResult);
    }
    ...
    auto shardToFull = rewriter.create<CustomCallOp>(
        loc, oldResult.getType(), shardingConstraint->getResult(0));
